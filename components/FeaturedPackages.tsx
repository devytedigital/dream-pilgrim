...
const FeaturedPackages = () => {
  ...
  return (
    <section
      ref={containerRef}
      className="py-20 md:py-32 bg-[#F9FAFB] text-[#111111] overflow-hidden flex flex-col items-center"
    >
      {/* Center Badges */}
      <div className="w-full px-6 md:px-12 flex justify-center gap-4 mb-16 md:mb-20">
      ...
      {/* Main Heading */}
      <div className="w-full px-6 md:px-12 mb-20 md:mb-32 flex justify-center items-center relative">
      ...
      <div className="w-full px-6 md:px-12 grid grid-cols-1 md:grid-cols-12 gap-16 md:gap-12 items-start">
        ...
        <div className="md:col-span-4 flex flex-col gap-8 md:justify-between pt-0 md:pt-12 text-center md:text-left items-center md:items-start order-1">
          <div className="space-y-6 md:space-y-8">
            <h3 className="text-4xl md:text-5xl font-normal leading-[1.1] tracking-tight">
              Not Your Boring <br /> Travel Agent
            </h3>
            <p className="text-gray-500 text-base md:text-lg max-w-[300px] leading-relaxed mx-auto md:mx-0">
              We plan chill, curated trips with good vibes and better people.
            </p>
          </div>

          <button className="group mt-8 md:mt-12 px-10 py-4 bg-[#111111] text-white rounded-2xl text-sm font-bold w-max transition-all cursor-pointer flex items-center justify-center gap-2 active:scale-95 shadow-xl shadow-[#111111]/10">
            Book a Seat
          </button>
        </div>

        {/* Right Side: Featured Packages Cards (Bottom on Mobile) */}
        <div className="md:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-6 order-2">
          {/* Card 1 */}
          <motion.div
            style={{ y: isDesktop ? y1 : 0 }}
            className="flex flex-col gap-4"
          >
            <div className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden group shadow-xl shadow-[#111111]/5">
              <Image
              ...
              <span className="absolute bottom-6 right-6 text-white text-[10px] font-medium px-4 py-2 bg-[#111111]/30 backdrop-blur-md rounded-full border border-white/10 uppercase tracking-widest">
                {packages[0].location}
              </span>
            </div>
            <div className="px-2">
              <h4 className="text-xl md:text-2xl font-semibold tracking-tight mb-1">
                {packages[0].title}
              </h4>
              <p className="text-gray-400 text-sm">{packages[0].subtitle}</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="px-2">
              ...
              <p className="text-gray-400 text-sm">{packages[1].subtitle}</p>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="px-2">
              ...
              <p className="text-gray-400 text-sm">{packages[2].subtitle}</p>
            </div>
          </motion.div>
        </div>
      </div>
